Feature: has_distinct_values_equal_set
  Background:
  ...
  Scenario: returns True
    Given a table named "input"
      ...
      | 1  | 1     | a   |
      | 1  | 2     | a   |
    And parameters "{"cols": "id", "values": [0, 1]}"
    When we try doing "has_distinct_values_equal_set" on table "input" and save the result to "output"
  ...
  Scenario: returns False
    Given a table named "input"
      ...
      | 1  | 1     | a   |
      | 1  | 2     | a   |
    And parameters "{"cols": "id", "values": [0, 1, 2]}"
    When we try doing "has_distinct_values_equal_set" on table "input" and save the result to "output"
  ...
  Scenario: returns False, several cols
    Given a table named "input"
      ...
      | 1  | 1     | a   |
      | 1  | 2     | a   |
    And parameters "{"cols": ["id", "bis"], "values": [0, 1, 2, "a"]}"
    When we try doing "has_distinct_values_equal_set" on table "input" and save the result to "output"
    Then the "output" equals to "<FALSE>"
